...
name: simple-english
description: |
  Write or rewrite ASD-STE100 Simplified Technical English for this repo's
  own documentation and process artifacts: everything under docs/,
  CONTRIBUTING.md, CHANGELOG.md, docs/decisions.md, commit messages, and PR
  descriptions. Use when asked to write or check these, or when the user
  says "STE", "Simplified Technical English", "ASD-STE100", or "de-slop".
  Never for src/content/ (narrative dialogue/content JSON), narrative prose
  discussion, code, or code comments — out of scope by design, see Project
  Scope below.
license: MIT
...
metadata:
  ...
  source: https://github.com/AminBlg/SimpleEnglish (vendored, MIT license — see LICENSE in this directory)
---

# Simple English: Write Like an Aerospace Manual

Vendored from [AminBlg/SimpleEnglish](https://github.com/AminBlg/SimpleEnglish)
v1.3.0 (MIT), with a Broadsheet & Rapier-specific **Project Scope** section
added below. The rule catalog, examples, and self-check that follow are
...
## Project Scope (Broadsheet & Rapier)

This skill applies **only** to this repo's own process documentation and
process artifacts:

- Everything under `docs/` (`game-design-spec.md`, `web-implementation.md`,
  `engine.md`, `feature-workflow.md`, `review-standards.md`,
  `docs/features/*.md`, etc.)
- `CONTRIBUTING.md`
- `CHANGELOG.md`
- `docs/decisions.md`
- Commit messages
...
- `src/content/` — every file in this directory is narrative dialogue or
  narrative content JSON (settlements, districts, POIs, actors, endeavors,
  dialogues). This is creative fiction, not technical documentation; STE's
  rules (one word one meaning, no synonym variation, controlled vocabulary)
  would flatten voice and tone that's deliberate.
- Any narrative prose discussion — worldbuilding, tone, character voice,
  or lore conversation, wherever it happens.
- Code or code comments anywhere in `src/`.

**On enforcement, read honestly.** Claude Code has no native mechanism to
technically restrict a skill to specific file paths — confirmed by checking
Anthropic's own skill-development reference documentation: `SKILL.md`
frontmatter supports only `name`/`description`/optional `version`/
`license`/`compatibility`/`metadata`, and plugin manifests have no scoping
field either. This Project Scope section, and the narrowed `description`
above, are the practical ceiling: they shape when this skill triggers and
what it claims as its territory, but nothing here is a hard technical
block. Treat a Simple-English-style edit proposed inside `src/content/`, on
code, or on narrative prose as a signal this skill has been misapplied —
reject it on sight, don't assume this note already prevented it.
