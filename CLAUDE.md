...
- **GitHub interaction:** `gh` CLI is available locally — prefer it over hand-rolled `curl`/REST polling scripts for anything GitHub-shaped (CI/workflow-run status, PRs, issues). It may not be on a given shell session's `PATH` if that session started before `gh` was installed on the machine — check with `gh --version` first, and if not found, locate the installed executable directly (e.g. via the OS package manager's install path) rather than falling back to a custom script. This repository is **public** — never commit account names, tokens, emails, local file-system paths, or any other machine/account-identifying detail into tracked files, even in passing (docs, comments, commit messages). Treat every file under this repo as world-readable before writing to it.
