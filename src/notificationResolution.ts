...
import { formatCurrencyDelta } from "./engine/store/notifications";
...
export function resolveNotificationMessage(
  ...
  switch (event.kind) {
    case "CURRENCY":
      return { id: event.id, tone: event.tone, message: formatCurrencyDelta(event.deltaBronze) };
    case "ITEM": {
    ...
    case "REPUTATION": {
      ...
      return { id: event.id, tone: event.tone, message: `${name} reputation ${sign}${event.amount}` };
    }
    case "ENDEAVOR_COMPLETE": {
      const title = ctx.endeavorTitles[event.endeavorId] ?? event.endeavorId;
      return { id: event.id, tone: event.tone, message: `Completed: ${title}` };
    }
  }
}
