...
describe("initial state", () => {
  it("matches the documented PlayerState shape", () => {
    ...
    expect(state.worldClock).toEqual({ shift: "MORNING", day: 1, season: "SPRING" });
...
describe("dispatchCommand", () => {
  ...
  it("rolls NIGHT over into the next day's MORNING", () => {
    usePlayerStore.setState({ worldClock: { shift: "NIGHT", day: 1, season: "SPRING" } });
  ...
  it("throws and leaves state untouched if COMMAND_NEXT_DAY is dispatched directly", () => {
    ...
    expect(usePlayerStore.getState().worldClock).toEqual({ shift: "MORNING", day: 1, season: "SPRING" });
  });
});
...
describe("parseAndValidateSave", () => {
  const populatedSave = {
    ...
    worldClock: { shift: "EVENING", day: 3, season: "AUTUMN" },
    currentLocation: {
