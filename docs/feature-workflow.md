...
- **Dialogue speaker reference: every new `DialogueNode` whose speaker is a real Actor sets `speakerActorId`, not just `speaker`.** `speaker` alone is a free-text display label with no referential check; `speakerActorId` is what `App.tsx`'s portrait resolution actually prefers and what `content-integrity.test.ts` validates against `Actor` (see `docs/features/feature_dialogue_speaker_reference.md`). Leave it unset only for a narration-style node with no Actor to reference (e.g. `"speaker": "Narration"`).
...
Beyond schema validation, this file also checks that cross-references between content files actually resolve: `Actor.factionIds` → existing `Faction`, `POI.actorIds` → existing `Actor` (and the reverse: that `Actor`'s `poiId` points back), `District.poiIds` → existing `POI` (and the reverse: that `POI`'s `districtId` points back), `Actor.dialogueId` → existing `Dialogue`, and `DialogueNode.speakerActorId` → existing `Actor` (when set — see `docs/features/feature_dialogue_speaker_reference.md`). This catches a **broken reference** automatically — a typo'd id, a deleted file nothing else was updated for.
...
Scoped deliberately to the relationships above. Other reference-shaped fields (`controllingFactionId`, `factionInfluence` keys, `District.settlementId`, `Endeavor.unlocksNodesOnComplete`) are equally checkable in principle but out of scope for this pass — noted so the gap is explicit, not silently assumed covered.
