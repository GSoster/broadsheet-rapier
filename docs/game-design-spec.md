...
This game has no character-power progression — no levels, no stat growth, no gear tiers where a later item is simply better than an earlier one. The player character does not get mechanically stronger over time. This is a deliberate choice, not an oversight: the project's own primary narrative influence (Fafhrd and the Gray Mouser, `docs/narrative-inspirations.md`) is defined by protagonists who accumulate story, reputation, and possessions — frequently losing the latter — without ever becoming numerically stronger.

Three legitimate forms of progression exist instead, and every future system must fit one of them:

1. World/state progression (implemented): unlocking Districts, POIs, Actors, and Endeavors (§6) as the player interacts with the world. This is the primary progress signal.
2. Item variety, not item power (implemented, §9/Open Design Gap #11): items express playstyle tradeoffs (a rapier trading damage for guard), never a straightforward "better than the last one." A future item that is simply strictly superior to an earlier one without a tradeoff violates this principle.
3. Collection and management systems (not yet built — e.g. a future fishing catalog, a future village-management system): these may exist, but whatever they produce must cash out as MORE ACCESS (a new Endeavor unlocked, a new POI/Actor introduced, a new item FLAVOR made available) or pure collection/flavor — never as character power, a stat increase, or anything that makes combat/minigames mechanically easier in a numeric sense. A village-management system that lets accumulated resources buy stat upgrades or cheaper duels would violate this principle even if framed as "unlocking a building."

Any future spec proposing a system that grants numeric character power must be rejected or redesigned to fit one of the three forms above, not treated as a new fourth category.

## 8. Faction & Reputation Model
...
1. Minigame resolution mechanics — formulas, difficulty scaling, win conditions — for `LOCKPICKING` and `FISHING` (`DICE` and `DUEL` are now resolved, see §10). `MinigameLauncherPayload.config` is now a discriminated union keyed off `type` covering `DiceConfig | DuelConfig` (see `web-implementation.md` §4); `LOCKPICKING`/`FISHING` still fall back to an untyped `Record<string, unknown>` until their mechanics are defined, at which point they should join the union as `LockpickingConfig | FishingConfig` too.
2. Reputation tiers and their gameplay effects. Must resolve within §7's Progression Philosophy: a reputation tier may unlock ACCESS (a discount, a new dialogue option, a new POI action) but must never grant raw numeric character power.
...
9. Systemic progression design — repeatable/always-available activities (see the dice minigame as a first instance) versus unique Endeavor content, avoiding soft-locks, and how repeated player actions should be acknowledged narratively rather than silently grinding. Reference: `docs/narrative-inspirations.md` Section 4 (Fallen London — including its design-principles subsection — OGame, BiteFight). Not yet designed as a general system — needs its own spec before generalizing beyond the one existing gambling loop. **The smaller, related half of this gap is resolved:** the one-off `triggerPoiEntryEffects`/`triggerDistrictEntryEffects` functions in `App.tsx` are now a typed on-enter-effects registry (`src/engine/utils/entryEffects.ts`, `EntryEffect` = `SOUND | DIALOGUE | START_ENDEAVOR`), generalized once a second real effect type (`EndeavorPhase`'s dialogue auto-trigger) actually existed, per the plan this gap already laid out, and later grew a third (`Endeavor`'s auto-start-on-enter, auto-starting an unstarted Endeavor on POI entry). Once three independently-shaped content fields existed for the same "when this condition is met, run these effects" idea (District/POI's `entrySoundAsset`, `EndeavorPhase.autoDialogueOnEnter`, `Endeavor.autoStartOnEnter`), they were unified into two shared, composable Zod schema fragments — `TriggerableSchema` (`onEnter: EntryEffect[]`, District/POI) and `PoiEntryTriggerSchema` (`{ poiId, onEnter: EntryEffect[] }`, `EndeavorPhase.onPoiEnter`/`Endeavor.onPoiEnter`) — see `docs/features/feature_dialogue_visibility_and_auto_triggers.md` (original generalization) and `docs/features/feature_triggerable_effects.md` (the field-shape unification). The larger "systemic progression design" question above is untouched by this and remains fully open. The larger "systemic progression design" question above is untouched by this and remains fully open.
