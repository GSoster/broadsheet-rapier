...
- State updates follow a CQRS pattern: UI components dispatch explicit `StateCommand` objects. Command handlers in the store are pure functions that return a new `PlayerState` (immutable, spread-based — never literal mutation) and emit state-change events (in-memory log, not persisted).
...
- **Endeavor**: `id`, `title`, `description`, `isUnlocked`, `initialPhaseId`, `phases: Record<string, EndeavorPhase>`, `onPoiEnter?: { poiId: string; onEnter: EntryEffect[] }` (via `PoiEntryTriggerSchema` — auto-starts this Endeavor when the player enters `poiId`, for an Endeavor not yet active; gated on `isNodeUnlocked`; the engine always synthesizes a `START_ENDEAVOR` effect from this Endeavor's own `id`/`initialPhaseId`, never content-authored in `onEnter` itself — see `docs/features/feature_triggerable_effects.md`).
- **EndeavorPhase**: `id`, `objectiveText`, `requiredClues?: string[]`, `nextPhaseOnSuccess?: string`, `unlocksNodesOnComplete: string[]`, `onPoiEnter?: { poiId: string; onEnter: EntryEffect[] }` (same `PoiEntryTriggerSchema` shape — runs `onEnter` when the player enters `poiId` while this phase is active; typically a `DIALOGUE` effect — `docs/features/feature_triggerable_effects.md`).
...
- Filenames: `snake_case` (e.g. `lantern_ward_bg.webp`). The extension itself is **not enforced anywhere** — `imageAsset` is a plain `z.string()` field and a `SOUND` effect's `asset` field is a plain `z.string()` within `EntryEffectSchema` (`shared.ts`/`engine/types`, no format/extension validation), and `AssetFallback`/`playSound` pass the path straight through to `<img src>`/`new Audio()`, format-agnostic. `.webp` in examples throughout this doc and the starter content is a convention from when those were placeholder filenames with no real file behind them yet, not a requirement — `.jpg`/`.png`/whatever the source art actually is works identically.
